...
@DisplayName("판매자 상품 삭제 - DELETE /api/seller/products/{productId}")
public class ProductDeleteTest extends MockMvcTest {
	...
	void 상품_삭제_성공__200_OK() throws Exception {
		...
		// when
		ResultActions actions = mockMvc.perform(delete("/api/seller/products/{productId}", product.getId())
		...
		ProductEntity deletedProduct = productRepository.findById(product.getId()).orElseThrow();
